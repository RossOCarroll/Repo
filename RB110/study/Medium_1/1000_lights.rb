...
3, 6, 9, and so on. You repeat this process and keep going until you have been through n repetitions.
...
-Output
  -An array of strings indicating which lights are on or off

Explicit:
  -Return an array

Implicit:
  -Return an array of numbers that represent which lights are on
...
Algorithm:
  -Create a hash with the keys from 1 to n number and the values as the string 'on'
  -Set a variable counter to 0
  -Iterate over the hash keys n time - 1
    -at counter transform value to 'off'
    -increment counter by 1
  -Return the hash keys if value is 'on'
