...
in the register. Thus, if we start with a stack of 3 6 4 (where 4 is the topmost item in the 
  stack), and a register value of 7, then the MULT operation will transform things to 3 6 on 
  the stack (the 4 is removed), and the result of the multiplication, 28, is left in the 
...
Input:
  -Strings that represent comands to the stack - regester

Ouput:
  -Integers that represent comands to the stack trace
...
Algorithm
    -Take a comand string or input string?
    -Create a regester that will hold and integer
    -Create an array the will represent a 'stack'
    -Iterate through the comand string
    -Depending on what the comand is
      -n, PUSH, ADD, SUB, MULT ecit
      -from the regester and stack

=end

def comands(comand, number = 0)
...
def minilang(command)
  ...
  command.split.each do |element|
    case element
    when /\A[-+]?\d+\z/ # Check if the element is an integer
