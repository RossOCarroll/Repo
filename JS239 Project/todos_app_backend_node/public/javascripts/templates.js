const templater = {
    ...
    return `
      <input type="checkbox" id="sidebar_toggle" />
        <div id="sidebar">
          <section id="all">
            <div id="all_todos">
              ${this.allTodosTemplate(todosData)}
            </div>
            <article id="all_lists">
              ${this.allListTemplate(todosData)}
            </article>
          </section>
          <section class="completed" id="completed_items">
            <div id="completed_todos">
              ${this.completedTodosTemplate(todosData)}
            </div>
            <article id="completed_lists">
              ${this.completedListTemplate(todosData)}
            </article>
          </section>
        </div>
      <div id="items">
  ...
  allTodosTemplate({ todos, current_section, active_date_all }) {
    const isActive = !active_date_all && current_section.title === 'All Todos';
    return `
      <header data-title="All Todos" data-total="${todos.length}" id="all_header" class="${isActive ? 'active' : ''}">
        <dl>
          <dt>All Todos</dt><!--
       --><dd>${todos.length}</dd>
        </dl>
  ...
  completedTodosTemplate({ done, current_section, active_date_completed }) {
    const isActive = !active_date_completed && current_section.title === 'Completed';
    return `
      <header data-title="Completed" data-total="${done.length}" id="all_done_header" class="${isActive ? 'active' : ''}">
        <dl>
          <dt>Completed</dt><!--
       --><dd>${done.length}</dd>
        </dl>
  ...
  allListTemplate({ todos_by_date, active_date_all }) {
    return Object.entries(todos_by_date)
      .map(([date, todosArr]) => `
        <dl data-title="${date}" data-total="${todosArr.length}" class="${date === active_date_all ? 'active' : ''}">
  ...
  completedListTemplate({ done_todos_by_date, active_date_completed }) {
    return Object.entries(done_todos_by_date)
      .map(([date, todosArr]) => `
        <dl data-title="${date}" data-total="${todosArr.length}" class="${date === active_date_completed ? 'active' : ''}">
          <dt><time>${date}</time></dt>
          <dd>${todosArr.length}</dd>
        </dl>
      `).join('');
