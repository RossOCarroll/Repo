/*
Given the head of a singly linked list containing integers, create a new linked list where each node 
represents the frequency of a unique element from the original list. The order of nodes in the new list 
doesn't matter. Return the head of this new frequency list.
...
Algorithm:
  - create a new linked list pass in 0
  - declare a current = head
  - declare a counts object
  - Loop while current is not null

    - if current val is not in counts
      - add current val as the value and 1 as the key
    - else 
      - increment the value at current val
  - Get the values from count
...
function createFrequencyList(head) {
  ...
  return createLinkedList(Object.values(counts));
}
