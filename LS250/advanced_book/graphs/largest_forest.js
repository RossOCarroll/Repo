...
/*
Algorithm:
  - create an empty array to add forest sizes too
  - create a visited set
  - Iterate over the rows
    - Iterate over the cols
      - if the current square has a 'T'
        - We check the trees around
        - find the number of trees and add to the tree array
  - retun the max from the forrest array
...
function largestForestArea(grid) {
  ...
  for (let row = 0; row < grid.length; row++) {
    for (let col = 0; col < grid[0].length; col++) {
      if (!visited.has(`(${row}, ${col})`) && grid[row][col] === 'T') {
        let size = dfs(row, col, grid, visited);
