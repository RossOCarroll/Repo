...
Algorithm:
  - isUnique
    - create a set with input arr
    - compare with input array length
...
function combinations(candidates, k) {
  let result = [];
  function backtrack(start, candidate) {
    if (candidate.length === k) { /* <<success condition>> */
    ...
    for (let i = start; i < candidates.length; i++) {
      ...
      backtrack(i + 1, candidate);  // explore
