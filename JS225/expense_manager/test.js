import { Expense, ExpenseManager, BudgetExpenseManager } from './expense_manager.js'
// Testing Helpers
function pass(msg) {
...
function fail(msg) {
  console.log('\x1b[32m FAIL:\x1b[0m ' + msg);
}
...
try {
  console.log('- `Expense` objects are immutable once created.')
  const expense = new Expense(1, 110, '2030-11-06', 'food');
  expense.id = 42;
  fail('should throw error when trying to update a property.')
} catch(_) {
  pass('cannot change properties once expense is created.')
}
...
try {
  ...
  pass('cannot add properties once expense is created.')
}
...
try {
  ...
  assert(manager.expenses.length === 0, 'expenses should have a length of 0');
...
try {
  ...
  pass('summarizes expenses (total, average, count)');
} catch(error) { fail(error.message) };
...
try {
  ...
  assert(foodExpenses.length === 2, 'Should be 2 food expenses');
  assert(foodExpenses[0].amount === 100, 'amount should equal to 100');
...
try {
  ...
  assert(expensesInRange.length === 2, 'should be 2 expenses');
  ...
  pass('filter by date range inclusive');
} catch(error) { fail(error.message) };
